...
| W09 | Implemented, smoke-scale evidence | [Multilingual wake profile](LANGUAGE-PROFILE.md): `model.language` accepts every token the pinned Whisper model supports and genuinely reaches inference (forced-Spanish transcription of an English clip) and status; the Spanish wake-corpus evidence is recorded (3/3 positives, 0 false activations / 0.32 h); other languages work through the same plumbing without per-language wake validation; real-room and human listening remain open |
